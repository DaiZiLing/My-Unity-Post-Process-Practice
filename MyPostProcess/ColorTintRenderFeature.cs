...
//【这一坨是DualKawase渲染】
    ...
    void Render(CommandBuffer cmd, ref RenderingData renderingData)
    {
        ...
        for(int i = 1; i < colorTint.RTDownSampling.value; i++)
        {
        ...
        for (int i = 0; i < colorTint.BlurTimes.value; i++)
        {
            ...
            cmd.Blit(source, downSampleRT[i], colorTintMaterial, 2);
            //cmd.Blit(downSampleRT[i], source);  
            source = downSampleRT[i];
        }

        //UpSample
        for (int i = colorTint.BlurTimes.value - 2; i >= 0; i--)
        {
            cmd.Blit(source, upSampleRT[i], colorTintMaterial, 3);
            //cmd.Blit(upSampleRT[i], source);  
            source = upSampleRT[i];
        }
