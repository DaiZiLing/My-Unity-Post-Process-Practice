    ...
    v2f_DualBlurUp DualBlurUpVert (appdata v)
{
    ...
    // float2 offset = float2(1 + _BlurRange, 1 + _BlurRange);
    // o.uv[0] = v.uv + float2(-_MainTex_TexelSize.x * 2, 0) * offset;
    // o.uv[1] = v.uv + float2(-_MainTex_TexelSize.x, _MainTex_TexelSize.y) * offset;
    // o.uv[2] = v.uv + float2(0, _MainTex_TexelSize.y * 2) * offset;
    // o.uv[3] = v.uv + _MainTex_TexelSize * offset;
    // o.uv[4] = v.uv + float2(_MainTex_TexelSize.x * 2, 0) * offset;
    // o.uv[5] = v.uv + float2(_MainTex_TexelSize.x, -_MainTex_TexelSize.y) * offset;
    // o.uv[6] = v.uv + float2(0, -_MainTex_TexelSize.y * 2) * offset;
    // o.uv[7] = v.uv - _MainTex_TexelSize * offset;
    //
    return o;
}
...
float4 DualBlurUpFrag (v2f_DualBlurUp i):SV_TARGET
{
    ...
    col += tex2D(_MainTex, i.uv[4] * 2) ;
    col += tex2D(_MainTex, i.uv[5] * 2) ;
    col += tex2D(_MainTex, i.uv[6] * 2) ;
    col += tex2D(_MainTex, i.uv[7] * 2) ;

    return col * 0.125; //sum / 12.0f
}
